...
def main():
    ...
    print("Creating monthly dual-pane layout...")
    ...
    day_font = load_font(DAY_FONT_PATH, HEADER_FONT_SIZE)
    time_font = load_font(TIME_FONT_PATH, 12)
    task_font = load_font(TASK_FONT_PATH, 12)
    ...
    draw.text((title_x, TITLE_PADDING), month_title, font=title_font, fill=BLACK)
    ...
    # Day-of-week headers (vertically centered in HEADER_HEIGHT, same as prototype_dual_pane.py)
    ...
    for day in range(1, total_days + 1):
        ...
        # Draw background color (all cells use GRAY2)
    ...
    # Right panel (40%): TODO square and date/time
    ...
    # TODO title aligned with week day headers (same as prototype_dual_pane.py)
    todo_title = "TODO"
    # Use same position as prototype_dual_pane.py: square_rect[1] + 6
    # square_start_y = grid_top, so TODO title at grid_top + 6
    draw.text((square_rect[0] + 6, square_rect[1] + 6), todo_title, font=day_font, fill=BLACK)

    # Sections as rows: Daily, Today, Upcoming
    sections = [
    ...
    section_font = load_font(DAY_FONT_PATH, 12)
    ...
    if USE_EINK_DISPLAY and EINK_AVAILABLE:
        ...
        # Export as PNG
        output_file = f"eink_monthly_dual_{today.strftime('%Y%m%d_%H%M%S')}.png"
