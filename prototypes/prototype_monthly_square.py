...
def main():
    ...
    for day in range(1, total_days + 1):
        ...
        # Hours as squares only (1h = 1 square, 0.5h = 1 square), 2 squares per line
        if hours > 0:
            # Round to nearest 0.5 hours
            rounded_hours = round(hours * 2) / 2.0
            # Each 0.5 hour = 1 square, so total squares = rounded_hours * 2
            total_squares = int(rounded_hours * 2)
            
            # Dimensions
            square_size = 5  # Square size (each square = 0.5 hour)
            spacing = 2  # Spacing between squares (left and right)
            row_spacing = 2  # Spacing between rows
            start_x = rect[2] - 6  # Right side of cell
            bottom_y = rect[3] - 4  # Start from bottom
            
            # Arrange squares in rows: 2 squares per line, stacked bottom to top
            row_y = bottom_y
            remaining_squares = total_squares
            
            while remaining_squares > 0:
                # Each row can have up to 2 squares
                squares_in_row = min(2, remaining_squares)
                
                # Draw squares from right to left
                x = start_x
    ...
    if USE_EINK_DISPLAY and EINK_AVAILABLE:
        ...
        # Export as PNG
        output_file = f"eink_monthly_v2_{today.strftime('%Y%m%d_%H%M%S')}.png"
