...
Simple function-based renderer for weekly timetable view
"""
...
def transform_tasks_to_weekly_format(api_todos: List[Dict], week_start_date: datetime) -> Dict[str, List]:
    ...
    # Normalize week_start_date to date only (remove time component)
    ...
    for task in api_todos:
        ...
        try:
            # Parse task date and normalize to date only
            task_date = datetime.strptime(task['start_date'], '%Y-%m-%d').date()
        except:
            continue
        
        # Check if task is within the week range (Monday to Sunday)
...
def render_weekly(data: Dict[str, Any], config: Dict[str, Any]) -> Image.Image:
    """
    Render weekly timetable view
    ...
    # Get Monday of current week (normalize to start of day)
    ...
    # Transform tasks (pass datetime, function will normalize to date)
    ...
    # Full width layout (no right panel for weekly view)
    table_start_x = TIME_COL_WIDTH
    table_width = width - table_start_x - PANEL_MARGIN
